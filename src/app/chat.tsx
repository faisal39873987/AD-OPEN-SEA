  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: inputValue,
          userId: isLoggedIn ? 'user-123' : 'anonymous', // Replace with actual user ID when available
        }),
      });
      ...
      // Check if we received services to display
      if (data.services && data.services.length > 0) {
        // Set selected service to the first match
        setSelectedService(data.services[0]);
        
        // If message is about payments, show payment modal
    ...
    <div className="flex flex-col h-screen bg-[#1a1a1a]">
      ...
      {isPaymentModalOpen && selectedService && (
        <PaymentModal
          ...
          user={null} // We'll add proper user management later
        />
      )}
    </div>
  );
